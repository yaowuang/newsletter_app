...
export const SectionTitleInput: React.FC<SectionTitleInputProps> = ({
  ...
  const commitTitle = (value: string) => {
    ...
    const isPlaceholder =
      existingContent === "" || existingContent === "- Your content here" || existingContent === "Your content here";
    if (targetedTitles.current.has(baseValue)) {
      if (isPlaceholder) {
        onUpdateContent(blockId, "content", generateWeekTable());
      } else if (!existingContent.includes("| Date | Event |")) {
        // avoid prompting if already a calendar table
  ...
  const handleKeyDown: React.KeyboardEventHandler<HTMLInputElement> = (e) => {
    ...
    if (open) {
      ...
      if (e.key === "ArrowDown") {
        ...
        e.preventDefault();
        setActive((i) =>
          filteredSuggestions.length ? (i - 1 + filteredSuggestions.length) % filteredSuggestions.length : -1,
        );
        ...
        <Dialog open={showCalendarPrompt} onOpenChange={setShowCalendarPrompt}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                You changed the title to a dates / events section. Do you want to replace the current content with a
                weekly calendar template?
              </DialogDescription>
            </DialogHeader>
            <DialogFooter className="gap-2 sm:gap-2">
              <Button type="button" variant="outline" onClick={() => setShowCalendarPrompt(false)}>
                Keep Content
              </Button>
              <Button
                type="button"
        ...
        <Input
          ...
          onChange={(e) => {
            ...
            if (onCaretChange) {
              ...
              onCaretChange(el.selectionStart ?? e.target.value.length);
            }
          }}
          onClick={(e) => {
            if (onCaretChange) {
            ...
            {filteredSuggestions.map((s: Suggestion, i: number) => (
              <li
                ...
                className={
                  ...
                  (i === active ? "bg-blue-600 text-white" : "hover:bg-gray-100 dark:hover:bg-gray-700")
                }
              >
