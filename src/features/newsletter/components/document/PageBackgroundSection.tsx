...
export const PageBackgroundSection: React.FC<PageBackgroundSectionProps> = ({
  ...
  const isCustomSize =
    (!!currentSize || editingCustomSize) && !sizeOptions.includes(currentSize) && currentSize !== DEFAULT_SENTINEL;
  const isCustomPosition =
    (!!currentPosition || editingCustomPosition) &&
    !positionOptions.includes(currentPosition) &&
    currentPosition !== DEFAULT_SENTINEL;
  const isCustomRepeat =
    (!!currentRepeat || editingCustomRepeat) &&
    !repeatOptions.includes(currentRepeat) &&
    currentRepeat !== DEFAULT_SENTINEL;
    ...
    <InspectorSection title="Page Background">
      <FormGroup label="Color" id={ids.color} inline>
        <Input
          id={ids.color}
          type="color"
          value={themeStyles.backgroundColor}
      ...
      <FormGroup label="Image" id={ids.color} inline>
        <div className="flex items-center gap-2">
          <ImageSourceDialog
            buttonText={themeStyles.backgroundImage ? "Change" : "Select"}
        ...
        <FormGroup label="Opacity" id={`${ids.color}-opacity`} inline>
          <div className="flex items-center gap-2 w-full">
            <input
              ...
              max={1}
              step={0.05}
              value={themeStyles.backgroundImageOpacity ?? 1}
              onChange={(e) => setBgOpacity?.(parseFloat(e.target.value))}
              className="flex-1"
            />
            <span className="text-xs w-10 text-right">
              {Math.round((themeStyles.backgroundImageOpacity ?? 1) * 100)}%
            </span>
          </div>
        </FormGroup>
      )}
      <FormGroup label="Size" id={ids.size} inline>
        <Select
          value={editingCustomSize || isCustomSize ? "CUSTOM" : currentSize ? currentSize : DEFAULT_SENTINEL}
          onValueChange={(v) => {
            if (v === "CUSTOM") {
      ...
      {(editingCustomSize || isCustomSize) && (
        <Input
          type="text"
          value={currentSize}
          placeholder="e.g. 160px 160px / cover"
      ...
      <FormGroup label="Position" id={ids.position} inline>
        <Select
          value={
            editingCustomPosition || isCustomPosition ? "CUSTOM" : currentPosition ? currentPosition : DEFAULT_SENTINEL
          }
          onValueChange={(v) => {
            if (v === "CUSTOM") {
      ...
      {(editingCustomPosition || isCustomPosition) && (
        <Input
          ...
          placeholder="e.g. 0 0, 80px 80px"
          onChange={(e) => setBgPosition?.(e.target.value)}
        />
      )}
      <FormGroup label="Repeat" id={ids.repeat} inline>
        <Select
          value={editingCustomRepeat || isCustomRepeat ? "CUSTOM" : currentRepeat ? currentRepeat : DEFAULT_SENTINEL}
          onValueChange={(v) => {
            if (v === "CUSTOM") {
      ...
      {(editingCustomRepeat || isCustomRepeat) && (
        <Input
          type="text"
          value={currentRepeat}
          placeholder="e.g. repeat, no-repeat"
