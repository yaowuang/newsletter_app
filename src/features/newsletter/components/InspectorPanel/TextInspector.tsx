...
import type { SectionStyleType, TextBlock } from "@/features/newsletter/types";
    ...
    <div className="space-y-6">
      ...
      <InspectorSection title="Title & Content">
        ...
        <FormGroup label="Section Content" id={`section-content-${block.id}`}>
          ...
          <Textarea
            ...
            onChange={(e) => {
              onUpdateTextBlock(block.id, "content", e.target.value);
              if (setEditingCaret) {
                setEditingCaret(block.id, "content", e.target.selectionStart ?? e.target.value.length);
              }
            }}
            onClick={(e) => {
              const el = e.currentTarget;
              if (setEditingCaret) {
                setEditingCaret(block.id, "content", el.selectionStart ?? 0);
              }
            }}
            onKeyUp={(e) => {
              const el = e.currentTarget;
      ...
      <InspectorSection title="Heading Styles">
        ...
        <FormGroup label="Heading Background" id={headingBgId} inline>
          <ColorInputWithReset
            ...
            onReset={() =>
              handleStyleChange("headingBackgroundColor", theme.styles.section.headingBackgroundColor || "#ffffff")
            }
          />
        </FormGroup>
        <FormGroup label="Heading Font" id={headingFontId} inline>
          <FontSelect
            id={headingFontId}
            value={currentStyle.headingFontFamily || theme.styles.section.headingFontFamily}
            onChange={(val) => handleStyleChange("headingFontFamily", val)}
            disabled={locked}
          />
        </FormGroup>
      </InspectorSection>
      ...
      <InspectorSection title="Content Styles">
        ...
        <FormGroup label="Content Font" id={contentFontId} inline>
          <FontSelect
            id={contentFontId}
            value={currentStyle.fontFamily || theme.styles.section.contentFontFamily}
      ...
      <InspectorSection title="Border Styles">
        ...
        <FormGroup label="Border Radius" id={borderRadiusId} inline>
          <Input
            id={borderRadiusId}
            type="number"
            value={currentStyle.borderRadius ?? theme.styles.section.borderRadius ?? 0}
            disabled={locked}
