...
/**
 * DateInspector displays and edits the content for a specific calendar date cell.
 * Allows formatting and emoji insertion for cell notes.
 */
...
export const DateInspector: React.FC<DateInspectorProps> = ({ dateKey, date, onClose }) => {
  ...
  const handleApplyFormatting = (action: FormattingAction) => {
    const current = typeof cellContent === 'string' ? cellContent : '';
    const el = textareaRef.current;
    if (!el) return;
    ...
    <div className="space-y-4">
      ...
      <InspectorSection title="Content">
        <FormGroup label="" id="cell-content">
          <FormattingToolbar onAction={handleApplyFormatting} />
          <Textarea
            id="cell-content"
            ref={textareaRef}
            value={cellContent}
          ...
          />
          <EmojiToolbar onInsert={handleInsertToken} />
        </FormGroup>
      </InspectorSection>
    </div>
  );
