...
import type { ImageElementType, SectionStyleType, TextBlockType } from "@/features/newsletter/types";
import { useStore } from "@/lib/store/index";

type SelectableElement =
  | (TextBlockType & { subType?: "title" | "content" })
  | ImageElementType
  | {
      id: string;
      type: "horizontalLine" | "calendarDate";
...
export function InspectorPanel({
  ...
  React.useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (selectedElement.type === "text") {
        const block = textBlocks.find((b: TextBlockType) => b.id === selectedElement.id);
        ...
        const targetId =
          (selectedElement as unknown as { subType?: string }).subType === "title"
            ? targetTitleId
  ...
  const getInspectorContent = () => {
    ...
    if (selectedElement.type === "calendarDate") {
      ...
      return <DateInspector dateKey={dateKey} date={date} onClose={() => selectElement(null)} />;
      ...
      return <ImageInspector image={selectedElement as ImageElementType} onUpdateImage={onUpdateImage} />;
