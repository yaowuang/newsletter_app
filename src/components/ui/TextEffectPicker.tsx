...
import { getTextEffectById, type TextEffect, textEffects } from "@/lib/textEffects";
...
export const TextEffectPicker: React.FC<TextEffectPickerProps> = ({ value, onChange, className }) => {
  const groupedEffects = textEffects.reduce(
    (acc, effect) => {
    ...
    <Select value={value || "none"} onValueChange={(val) => onChange(val === "none" ? undefined : val)}>
      <SelectTrigger className={className}>
        <SelectValue placeholder="Choose text effect">
          {value ? <TextEffectPreview effectId={value} /> : "Solid Color"}
        </SelectValue>
      </SelectTrigger>
      <SelectContent>
...
const TextEffectPreview: React.FC<TextEffectPreviewProps> = ({ effectId, showSample = false }) => {
  ...
  return (
    <div className="flex items-center gap-2">
      {showSample &&
        (isPastel ? (
          <PastelRotateText text="Pastel" className="text-sm font-bold inline-flex gap-px" />
        ) : isRainbow ? (
          <RainbowRotateText text="Rainbow" className="text-sm font-bold inline-flex gap-px" />
        ) : (
          <span className="text-sm font-bold" style={effect.styles}>
            Aa
          </span>
        ))}
