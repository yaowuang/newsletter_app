...
import { allLayouts, type LayoutType, type LayoutVariantType } from "@/features/newsletter/utils/layouts";
import { cn } from "@/lib/utils";
import type { LayoutPickerProps } from "./interfaces/picker-interfaces";
...
export const LayoutPicker: React.FC<LayoutPickerProps> = ({
  ...
  const availableLayouts =
    selectedCategory === "calendar" ? calendarLayouts : newsletterLayouts.filter((l) => l.sections === desiredSections);
  ...
  return (
    <div className="space-y-4">
      ...
      {selectedCategory !== "calendar" && (
        <SectionCountSelector selectedCount={desiredSections} onCountChange={handleSectionChange} />
      )}
...
const CategorySelector: React.FC<{
  ...
  <div>
    ...
    <div role="group" aria-label="Layout Category" className="flex items-center justify-center gap-2 mt-2">
      <Button
              ...
              {layout.variants.map((variant) => (
                ...
                >
                  <LayoutPreview
                    ...
                    isSelected={
                      currentSelection.variant.name === variant.name && currentSelection.base.id === layout.id
                    }
                  />
                </div>
              ))}
...
const LayoutPreview: React.FC<{
  ...
  const variant =
    layout.variants.find((v) => v.name === (variantName || layout.variants[0].name)) || layout.variants[0];
  const areas = [...new Set(layout.gridTemplateAreas.replace(/"/g, "").split(/\s+/).filter(Boolean))];

  return (
    <div
      className={cn("p-2 rounded-lg cursor-pointer w-[90px]", {
        "bg-blue-100 dark:bg-blue-900 ring-1 ring-blue-500": isSelected,
      })}
    >
      <LayoutPreviewGrid layout={layout} variant={variant} areas={areas} />
      <LayoutPreviewLabel layoutName={layout.name} variantName={variant.name} />
    </div>
  );
};
  ...
  return (
    <div
      style={{
        ...
        gridTemplateRows: variant.gridTemplateRows.replace(/auto/g, "10px").replace(/(\d+)fr/g, "$1fr"),
        gap: "3px",
        width: "80px",
        height: "90px",
        border: "1px solid #ccc",
    ...
    >
      {areas.map((area) => (
        <LayoutPreviewArea key={area} area={area} variant={variant} />
      ))}
    </div>
  );
};
...
const LayoutPreviewArea: React.FC<{
  ...
  const getLabel = () => {
    if (area === "title") {
      return variant.titleAlign === "center" ? "T" : variant.titleAlign === "right" ? "T>" : "<T";
    }
    if (area === "date") {
      return variant.dateAlign === "center" ? "D" : variant.dateAlign === "right" ? "D>" : "<D";
    }
    if (isSection) {
