...
import type { ClipartSearchProps } from "./interfaces/picker-interfaces";
import {
...
export const ClipartSearch: React.FC<ClipartSearchProps> = ({ onResultSelect }) => {
  const [searchService] = useState<ImageSearchService | null>(() => ImageSearchServiceFactory.createPixabayService());
...
const SearchControls: React.FC<{
  ...
  <div className="flex gap-2 items-center">
    ...
    <Button size="sm" onClick={onSearch} disabled={loading || !query.trim()} aria-label="Search clipart">
      {loading ? "..." : "Search"}
    </Button>
  </div>
);
...
const ApiKeyWarning: React.FC = () => (
  <p className="text-xs text-amber-600">Set NEXT_PUBLIC_PIXABAY_KEY to enable search.</p>
);

const ErrorMessage: React.FC<{ message: string }> = ({ message }) => <p className="text-xs text-red-600">{message}</p>;
...
const SearchResults: React.FC<{
  ...
  <div className="min-h-[140px] border rounded-md p-2 overflow-auto">
    ...
    {!loading && results.length === 0 && !hasValidApiKey && (
      <p className="text-xs text-muted-foreground">API key required for search.</p>
    )}

    {!loading && results.length === 0 && hasValidApiKey && (
      <p className="text-xs text-muted-foreground">No results. Try another term.</p>
    )}

    {!loading && results.length > 0 && (
      <div className="grid grid-cols-4 gap-2">
        {results.map((result) => (
          <SearchResultItem key={result.id} result={result} onClick={() => onResultClick(result)} />
        ))}
      </div>
    )}
  </div>
...
const SearchResultItem: React.FC<{
  ...
  >
    {result.thumbnail ? (
      ...
      <img src={result.thumbnail} alt={result.title} className="w-full h-14 object-contain" loading="lazy" />
    ) : (
      <div className="w-full h-14 flex items-center justify-center text-[10px] text-muted-foreground">No Img</div>
    )}
    <p className="text-[10px] mt-1 text-center line-clamp-2 leading-tight" title={result.title}>
      {result.title}
    </p>
