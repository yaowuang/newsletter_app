    ...
    <InspectorSection title="Title & Date">
      ...
      <FormGroup label="Date Mode" id={dateInputId} inline>
        <Select value={dateMode} onValueChange={(v: DateMode) => {
          ...
          if (v === 'month') { onDateChange(currentMonth); }
          else if (dateMode === 'month' && v === 'single') { onDateChange(`${currentMonth}-01`); }
            else if (dateMode === 'month' && v === 'week') { onDateChange(computeBusinessWeekRange(`${currentMonth}-01`)); }
            else if (v === 'week' && dateMode === 'single') { onDateChange(computeBusinessWeekRange(currentDayISO)); }
            else if (v === 'single' && dateMode === 'week') { onDateChange(currentDayISO); }
          onDateModeChange(v);
        }}>
      ...
      {dateMode === 'month' && (
        <FormGroup label="Month" id={`${dateInputId}-month`}>
          <Input id={dateInputId} name="newsletterMonth" type="month" value={/^[0-9]{4}-[0-9]{2}$/.test(date) ? date : (dateInputValue ? dateInputValue.slice(0,7) : '')} onChange={e => handleDateChange(e.target.value)} />
        </FormGroup>
      )}
    </InspectorSection>
  );
