...
import type { LayoutDecorationType } from "@/features/newsletter/utils/layouts";
...
export const createNewsletterSlice: StateCreator<NewsletterSliceType, [], [], NewsletterSliceType> = (set, get) => ({
    ...
    set((state) => {
      ...
      if (state.textBlockMap[blockId]) {
        return {
          ...
          sectionStyles: {
            ...state.sectionStyles,
            [blockId]: state.sectionStyles[blockId] || {},
          },
        };
      }
  ...
  setThemeTitleFont: (font) =>
    set((state) => ({
      theme: {
        ...state.theme,
        styles: {
          ...state.theme.styles,
          title: { ...state.theme.styles.title, fontFamily: font },
        },
      },
    })),
  setThemeDateFont: (font) =>
    set((state) => ({
      theme: {
        ...state.theme,
        styles: {
          ...state.theme.styles,
          date: { ...state.theme.styles.date, fontFamily: font },
        },
      },
    })),
  setThemeTitleColor: (color) =>
    set((state) => {
      ...
      return {
        theme: {
          ...state.theme,
          styles: { ...state.theme.styles, title: { ...currentTitle, color } },
        },
      };
    }),
  setThemeDateColor: (color) =>
    set((state) => ({
      theme: {
        ...state.theme,
        styles: {
          ...state.theme.styles,
          date: { ...state.theme.styles.date, color },
        },
      },
    })),
  setThemeTitleAlignment: (align: "left" | "center" | "right") =>
    set((state) => ({
      theme: {
        ...state.theme,
        styles: {
          ...state.theme.styles,
          title: { ...state.theme.styles.title, textAlign: align },
        },
      },
    })),
  setThemeDateAlignment: (align: "left" | "center" | "right") =>
    set((state) => ({
      theme: {
        ...state.theme,
        styles: {
          ...state.theme.styles,
          date: { ...state.theme.styles.date, textAlign: align },
        },
      },
    })),
  setThemeTitleTextEffect: (effectId) =>
    set((state) => {
      const currentTitle = state.theme.styles.title;
      let newTitleStyles;
      if (!effectId) {
  ...
  setThemePageBackgroundColor: (color) =>
    set((state) => ({
      theme: {
        ...state.theme,
        styles: {
          ...state.theme.styles,
          page: { ...state.theme.styles.page, backgroundColor: color },
        },
      },
    })),
  setThemePageBackgroundImage: (image) =>
    set((state) => ({
      theme: {
        ...state.theme,
        styles: {
          ...state.theme.styles,
          page: {
            ...state.theme.styles.page,
            backgroundImage: image || undefined,
          },
        },
      },
    })),
  setThemePageBackgroundSize: (size) =>
    set((state) => ({
      theme: {
        ...state.theme,
        styles: {
  ...
  setThemePageBackgroundPosition: (position) =>
    set((state) => ({
      theme: {
        ...state.theme,
        styles: {
          ...
          page: {
            ...state.theme.styles.page,
            backgroundPosition: position || undefined,
          },
        },
      },
    })),
  setThemePageBackgroundRepeat: (repeat) =>
    set((state) => ({
      theme: {
        ...state.theme,
        styles: {
  ...
  setThemePageBackgroundImageOpacity: (opacity) =>
    set((state) => ({
      theme: {
        ...state.theme,
        styles: {
          ...
          page: {
            ...state.theme.styles.page,
            backgroundImageOpacity: Math.min(1, Math.max(0, opacity)),
          },
        },
      },
    })),
  setLayout: (layout) => {
    ...
    const getStyleFromLibItem = (
      libItem: HorizontalLineStyle | null | undefined,
    ): "solid" | "dashed" | "dotted" | "clipart" | "shadow" => {
    ...
    set((state) => {
      ...
      if (variant.sectionCharTargets && variant.sectionCharTargets.length === state.textBlockOrder.length) {
        ...
        if (remaining.length) {
          const unfilled = variant.sectionCharTargets
            .map((_: unknown, i: number) => i)
            .filter((i: number) => !(i in assignment));
          unfilled.forEach((idx: number, ri: number) => {
            assignment[idx] = remaining[ri];
          });
        }
      ...
      const newAutoLines = decorations.map((dec: LayoutDecorationType) => {
        ...
        const existing: HorizontalLineElementType | undefined = (get() as RootStore).horizontalLines.find(
          (l: HorizontalLineElementType) => l.autoGenerated && l.decorationKey === key,
        );
        const libItem =
          dec.lineId === "themed"
            ? resolveThemedLine(theme.name)
            : horizontalLineLibrary.find((l: HorizontalLineStyle) => l.id === dec.lineId);
        ...
        const positionLookup: Record<string, () => { y: number; metrics: { x: number; width: number } }> = {
          afterTitle: () => ({
            y: titleEndY - 2,
            metrics: getLineMetrics(titleAlign, true),
          }),
          afterDate: () => ({
            y: dateEndY - 4,
            metrics: getLineMetrics(dateAlign, false),
          }),
          beforeSections: () => ({
            y: sectionsStartY - 8,
            metrics: getLineMetrics("center", false),
          }),
          betweenSections: () => ({
      ...
      const keptManual = (state as RootStore).horizontalLines.filter(
        (l: HorizontalLineElementType) => !l.autoGenerated,
      );
      ...
      const updatedTheme: ThemeType = {
        ...
        styles: {
          ...
          title: {
            ...theme.styles.title,
            textAlign: variant.titleAlign || "center",
          },
          date: {
            ...theme.styles.date,
            textAlign: variant.dateAlign || "center",
          },
        },
      };
      return {
        layout,
        horizontalLines: [...keptManual, ...newAutoLines],
        theme: updatedTheme,
  ...
  updateStyle: (blockId, newStyles) => {
    set((state) => {
      ...
      return {
        sectionStyles: {
          ...state.sectionStyles,
          [blockId]: { ...(state.sectionStyles[blockId] || {}), ...newStyles },
        },
      };
    });
  },
