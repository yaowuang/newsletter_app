...
import type { ImageElementType, TextBlockType } from "@/features/newsletter/types";
import { useStore } from "@/lib/store/index";

const StuffPanel = dynamic(() => import("@/components/ToolsPanel/StuffPanel").then((mod) => mod.StuffPanel), {
  ssr: false,
...
export default function BuilderPage() {
  ...
  useEffect(() => {
    const handleDeleteElement = (e: CustomEvent) => {
      ...
      if (detail?.id && (detail.type === "text" || detail.type === "image" || detail.type === "horizontalLine")) {
        if (detail.type === "text") {
  ...
  const selectedBlock = useMemo(() => {
    ...
    if (selectedElement.type === "text") {
      const base = textBlocks.find((b: TextBlockType) => b.id === selectedElement.id);
      const subType = (selectedElement as { subType?: string }).subType;
      const validSubType: "title" | "content" | undefined =
        subType === "title" || subType === "content" ? subType : undefined;
    ...
    <div className="flex flex-col h-screen min-h-0 relative">
      ...
      {(!showLeft || !showRight) && (
        <div className="flex items-center gap-3 px-2 py-1 bg-background/60 border-b border-border/50 backdrop-blur-sm">
          {!showLeft && (
            <Button size="sm" variant="outline" onClick={() => setShowLeft(true)}>
              Show Tools
            </Button>
          )}
          <div className="flex-1" />
          {!showRight && (
            <Button size="sm" variant="outline" onClick={() => setShowRight(true)}>
              Show Inspector
            </Button>
          )}
        </div>
      ...
      <ResizablePanelGroup direction="horizontal" className="flex-grow h-full min-h-0 max-w-screen">
          ...
          <>
            <ResizablePanel defaultSize={25} minSize={16} maxSize={32} className="flex flex-col min-h-0">
              <div className="flex items-center justify-between px-3 py-2 border-b border-border/50 bg-background/60 sticky top-0 z-10 backdrop-blur">
                ...
                <Button size="sm" variant="ghost" onClick={() => setShowLeft(false)} aria-label="Hide tools panel">
                  Hide
                </Button>
              </div>
        ...
        <ResizablePanel
          defaultSize={showLeft && showRight ? 50 : showLeft || showRight ? 70 : 100}
          ...
          <>
            ...
            <ResizablePanel defaultSize={25} minSize={16} maxSize={32} className="flex flex-col min-h-0">
              <div className="flex items-center justify-between px-3 py-2 border-b border-border/50 bg-background/60 sticky top-0 z-10 backdrop-blur">
                ...
                <Button size="sm" variant="ghost" onClick={() => setShowRight(false)} aria-label="Hide inspector panel">
                  Hide
                </Button>
              </div>
              <div className="flex-1 min-h-0 overflow-y-auto">
